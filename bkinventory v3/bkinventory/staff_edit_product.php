...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Get the current product_edit_by and product_last_edit_by values
    $query = "SELECT product_edit_by, product_last_edit_by FROM Products WHERE product_id='$productId'";
    $result = mysqli_query($con, $query);
    $row = mysqli_fetch_assoc($result);
    $currentEditBy = $row['product_edit_by'];
    $lastEditBy = $row['product_last_edit_by'];

    if ($currentEditBy && $currentEditBy != $userId) {
        // If there is a current editor and it's not the current user, update both fields
        ...
        // Update only the product_edit_by field
        $query = "UPDATE Products SET product_name='$productName', product_category='$category', product_size='$size', product_selling_price='$price', invested_price='$invested_price', product_stock_quantity='$stock', product_cups='$cups', product_status='$status', product_image='$product_image', product_edit_by='$userId', product_last_edit_by='$lastEditBy', product_last_edited=CURRENT_TIMESTAMP WHERE product_id='$productId'";
    }
